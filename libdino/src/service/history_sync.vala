...
//            debug(@"MAM: [%s] In range (time) %s < %s", account.bare_jid.to_string(), mam_flag.server_time.to_string(), catchup_until_time[account].to_string());
    ...
    public async bool fetch_mam(Conversation conversation, bool before = false) {
        ...
        debug("forwards_synced = ");
        foreach(var v in forwards_synced) {
            debug("have this in forwards_synced: %s", v.counterpart.to_string());
        }
        debug("/forwards_synced");
        if (mam_catchup == null) {
        ...
        try {
            ...
            if(!mam_cursors[conversation].contains(before)) {
                ...
                // subtle: we do not fire 'syncing' on the first sync, because
                // - it might be the degenerate no-archive case that is impossible to save
                // - there's a good chance it's a small result that can sync in one request
                //   and in that case it's messy to surface the sync to the user, it's just an
                //   annoying flicker for them on every convo switch.
                sent_syncing = true;
                syncing(before);
            }
    ...
    private async PageRequestResult get_mam_page(Conversation conversation, Xmpp.MessageArchiveManagement.V2.MamQueryParams query_params, PageRequestResult? prev_page_result, Cancellable? cancellable = null) {
        ...
        debug("We are in a %s, and we're updating mam_catchup.id = %d", page_result.page_result.to_string(), mam_catchup[db.mam_catchup.id]);
        var query = db.mam_catchup.update()
        ...
        if (query_params.start_id == null && query_params.end_id == null) {
            ...
            if(page_result.query_result.last != null && page_result.query_result.last != "") {
                query.set(db.mam_catchup.from_id, page_result.query_result.last); // the *last* stanza-id we were given is the outer *forward* ("to") edge of our region
            }
            // query.set(db.mam_catchup.to_time, ...); // TODO

            if (page_result.page_result == PageResult.NoMoreMessages || page_result.page_result == PageResult.TargetReached) {
                debug("BA");
            ...
            if(page_result.query_result.first != null && page_result.query_result.first != "") {
                query.set(db.mam_catchup.from_id, page_result.query_result.first);
            }
